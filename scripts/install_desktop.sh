...
# (The RPM install ships packaging/we-rsynk-ing.desktop instead, which
# uses the installed `we-rsynk-ing` entry point and needs no Path=.)
set -euo pipefail
...
sed -e 's|^Exec=.*|Exec=python -m rsync_app|' \
    -e '/^TryExec=/d' \
    -e "/^Exec=/a Path=$SRC" \
    "$SRC/packaging/we-rsynk-ing.desktop" \
    > "$HOME/.local/share/applications/we-rsynk-ing.desktop"
