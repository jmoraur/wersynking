...
# (The RPM install ships packaging/wersyncing.desktop instead, which
# uses the installed `wersyncing` entry point and needs no Path=.)
set -euo pipefail
...
sed -e 's|^Exec=.*|Exec=python -m rsync_app|' \
    -e '/^TryExec=/d' \
    -e "/^Exec=/a Path=$SRC" \
    "$SRC/packaging/wersyncing.desktop" \
    > "$HOME/.local/share/applications/wersyncing.desktop"
